...
def points_cloud_generater(color_img, xyz, uv):
    points_cloud = np.zeros((6, xyz.shape[0]))
    points_cloud[0] = xyz.T[0].reshape(-1)
    points_cloud[1] = xyz.T[1].reshape(-1)
    points_cloud[2] = xyz.T[2].reshape(-1)
    points_cloud[3] = color_img[uv.astype(int)[:, 0], uv.astype(int)[
        :, 1], 0].reshape(-1)
    points_cloud[4] = color_img[uv.astype(int)[:, 0], uv.astype(int)[
        :, 1], 1].reshape(-1)
    points_cloud[5] = color_img[uv.astype(int)[:, 0], uv.astype(int)[
        :, 1], 2].reshape(-1)
    ...
    points_colors = points_cloud[:, 3:6]
    # normalization
    if not ((points_colors >= 0.0) & (points_colors <= 1.0)).all():
        points_colors /= 255.0
    pcd.colors = o3d.utility.Vector3dVector(points_colors)

    return points_cloud.T
...
if __name__=="__main__":
    cnt = 0
    while 1:
        # ok,corners=findchessboardcorners(color,)
        if cnt % 100 == 0:
            color, _ = freenect.sync_get_video()
            depth, _ = freenect.sync_get_depth()
            xyz, uv = depth2xyzuv(depth)
            print(xyz.shape, xyz[xyz[:, 0] < 0], uv.astype(int)[:, 0])
            cv2.imshow('color', color[:, :, ::-1])
            cv2.imshow('depth', depth.astype(np.uint8))
            point_cloud = points_cloud_generater(color, xyz, uv)
